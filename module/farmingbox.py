...
def farmingbox(data, table_name):
    ...
    data.rename({4:"temperature"}, axis=1, inplace=True)
    data.rename({5:"humidity"}, axis=1, inplace=True)
    ...
    data["floor"] = data["status"].map(lambda x : status_spliter(x)[0])
    data["ratio"] = data["status"].map(lambda x : status_spliter(x)[1])
    data["solution"] = data["status"].map(lambda x : status_spliter(x)[2])
    # 원핫 인코딩 대신에 해당 str 값으로 수정함
    
    drop_category = ["time", "status", "empty"]
    data.drop(drop_category, axis=1, inplace=True)
